<template>
  ...
  <div class="actions w-full flex items-center justify-end gap-2 capitalize my-4">
    ...
    <InPlaceConfirmDelete v-if="isEdit" :onDelete="deleteCard" />
  </div>
</template>
...
import { Color, Card, NTagColor } from '@/data/types/mock';
import { useBoards, useLists, useCards } from '@/use/db';
...
export default defineComponent({
  ...
  setup() {
    ...
    const { sort } = useLists;
    const { data, add, update, del } = useCards;
    ...
    const list = computed(() => boardData.currentBoard?.lists?.find((l) => l.id === Number(route.params.listId)));
    ...
    function setFields() {
      if (!!edit.value) {
        const { position, text, label, details, color, text_color } = edit.value;
