    ...
    <form class="flex flex-col gap-2" @submit.prevent @keydown.enter.prevent>
      ...
      <div class="flex flex-wrap md:flex-nowrap md:space-x-4">
        ...
        <div class="form-group md:max-w-1/2">
          ...
          <div class="grid grid-cols-5 gap-2 py-1">
            <button v-for="c in getBackgroundColors" :key="c" @click.prevent="setBackgroundColor(c)">
...
export default defineComponent({
  ...
  setup(props) {
    ...
    function setFields() {
      if (props.edit) {
        const { position, text, details, color, text_color } = props.edit;
    ...
    function reset() {
      ...
      // state.audio_url= '';
      state.details = '';
      state.color = getBackgroundColors[3];
